...
void retroseq_dsp64(t_retroseq *x, t_object *dsp64, short *count, double sr, long n, long flags);
...
void ext_main(void *r)
{
	...
	c = retroseq_class;
	class_addmethod(c, (method)retroseq_dsp64, "dsp64", A_CANT, 0);
...
void retroseq_shuffle_durs(t_retroseq *x)
{
	...
	/* Copy the sequence to a list of Atoms (note replacement of SETFLOAT macro with atom_setfloat()) */
	
	for( i = 0; i < d_sequence_length; i++ ){
		atom_setfloat(pseq_list+i,d_sequence[i]);
	}
...
void retroseq_shuffle_freqs(t_retroseq *x)
{
	...
	for( i = 0; i < f_sequence_length; i++ ){
		atom_setfloat(pseq_list+i,f_sequence[i]);
	}
...
void retroseq_send_adsr(t_retroseq *x)
{
	...
	for(i = 0; i < 10; i++){ 
		atom_setfloat(adsr_list+i,adsr_out[i]);
	}
